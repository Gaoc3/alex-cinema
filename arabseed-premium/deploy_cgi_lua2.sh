...
io.stderr:write("HOST=" .. host .. " PORT=" .. port .. " PATH=" .. path .. "\n")
io.stderr:flush()
...
io.stderr:write("IP=" .. tostring(ip) .. "\n")
io.stderr:flush()
...
io.stderr:write("RECV buf type=" .. type(buf) .. " len=" .. tostring(#buf) .. "\n")
io.stderr:flush()

while buf and #buf > 0 do
  all[#all + 1] = buf
  local ok2, events = pcall(nixio.poll, {{fd = s, events = nixio.poll_flags("in")}}, 5000)
  if ok2 and events and events[1] then
    local rev = events[1].revents
    io.stderr:write("POLL rev=" .. rev .. "\n")
    io.stderr:flush()
    if rev % 2 >= 1 then
      buf = s:recv(16384)
      io.stderr:write("RECV more len=" .. tostring(#buf) .. "\n")
      io.stderr:flush()
    else
      break
    end
  else
    io.stderr:write("POLL timeout/error\n")
    io.stderr:flush()
    break
  end
end
...
io.stderr:write("TOTAL response len=" .. #response .. "\n")
io.stderr:write("RESPONSE=" .. response:sub(1, 500) .. "\n")
io.stderr:flush()
...
sleep 1
echo "=== Test ==="
uclient-fetch -q -O- "http://127.0.0.1/cgi-bin/proxy?url=https%3A%2F%2Fcinemana.shabakaty.com%2F" 2>/tmp/cgi-debug.log
echo "=== stderr ==="
