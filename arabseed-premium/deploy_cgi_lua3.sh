...
local function debug(msg)
  local f = io.open("/tmp/cgi-debug.log", "a")
...
if not proto then
  proto, hostport = url:match("(https?)://([^/]+)")
...
debug("HOST=" .. host .. " PORT=" .. port .. " PATH=" .. path)

local addrs, err = nixio.getaddrinfo(host, "inet", 0)
local ip
...
if not ok then
  debug("CONNECT FAILED: " .. tostring(err))
  s:close()
  io.write("Content-Type: text/plain\r\n\r\nERROR: connect to " .. host .. " failed: " .. tostring(err))
  return
end
debug("CONNECTED")
...
debug("RECV len=" .. tostring(#buf))

while buf and #buf > 0 do
  all[#all + 1] = buf
  local ok, events = pcall(nixio.poll, {{fd = s, events = nixio.poll_flags("in")}}, 5000)
  if ok and events and events[1] then
    local rev = events[1].revents
    if rev and rev % 2 >= 1 then
      buf = s:recv(16384)
      debug("RECV more len=" .. tostring(#buf))
    else
      debug("POLL rev=" .. tostring(rev) .. " stopping")
      break
    end
  else
    debug("POLL timeout/error ok=" .. tostring(ok))
    break
  end
end
s:close()
...
debug("TOTAL len=" .. #response)
debug("FIRST 300=" .. response:sub(1, 300))

local hdr_end = response:find("\r\n\r\n")
if not hdr_end then
  debug("NO HEADER END FOUND")
  io.write("Content-Type: text/plain\r\n\r\nERROR: no headers in response. Raw=" .. response:sub(1, 200))
  return
end

local resp_headers = response:sub(1, hdr_end - 1)
local body = response:sub(hdr_end + 4)
debug("HEADERS=" .. resp_headers)
...
sleep 1
echo "=== Test ==="
uclient-fetch -q -O- "http://127.0.0.1/cgi-bin/proxy?url=https%3A%2F%2Fcinemana.shabakaty.com%2F" 2>/dev/null
echo ""
echo "=== stderr ==="
cat /tmp/cgi-debug.log 2>/dev/null