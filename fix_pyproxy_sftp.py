...
sftp = ssh.open_sftp()
with sftp.file('/tmp/pyproxy_new.py', 'w') as f:
    f.write(new_pyproxy)
sftp.close()

# Move and restart
commands = [
    'dbclient -y -y -i /etc/dropbear/id_rsa root@64.225.99.144 "cp /tmp/pyproxy_new.py /opt/pyproxy.py && systemctl restart pyproxy && sleep 2 && systemctl is-active pyproxy"'
]
