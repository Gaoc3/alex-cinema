...
CMD="/tmp/usr/bin/curl --raw -s -k -i -L --http1.1"
...
for line in nph_proxy_content.splitlines():
    ...
    client.exec_command(f"echo '{escaped_line}' >> /tmp/nph-proxy")

client.exec_command('chmod +x /tmp/nph-proxy')
client.exec_command('ln -s /tmp/nph-proxy /www/cgi-bin/nph-proxy')
