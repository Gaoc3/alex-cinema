...
# Write to router via echo + base64 decode
cmd_write = f"echo '{b64_script}' | base64 -d > /tmp/vps_script.sh"
ssh.exec_command(cmd_write)

# Execute the script on the VPS through dbclient
command = 'cat /tmp/vps_script.sh | dbclient -y -y -i /etc/dropbear/id_rsa root@64.225.99.144 "sh -s"'
