...
server {
    ...
    # [AI ADAPTIVE ROUTE] - ALL CDNs map to the fallback working CDN: 127.0.0.1 on port 8084
    ...
    # [AI ADAPTIVE ROUTE] - Main API Route
    location /cinemana/ {
    ...
    # Fallback to 8081 for anything else (useful for misrouted assets)
    location / {
        proxy_pass https://127.0.0.1:8081;
        proxy_set_header Host "cinemana.shabakaty.com";
        proxy_ssl_server_name on;
...
sftp = c.open_sftp()
with sftp.file('/tmp/nginx_default.conf', 'w') as f:
    f.write(nginx_config)
sftp.close()

cmd = "dbclient -y -y -i /etc/dropbear/id_rsa root@64.225.99.144 'cat > /etc/nginx/sites-available/default && systemctl reload nginx' < /tmp/nginx_default.conf"
